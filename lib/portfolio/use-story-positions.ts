...
import type { ProductStoryNode } from './product-story-graph'

const STORAGE_KEY = 'jasmine-product-story-positions'

type Position = { x: number; y: number }
type PositionMap = Record<string, Position>

function loadPositions(nodes: ProductStoryNode[]): PositionMap {
...
export function useStoryPositions(
  nodes: ProductStoryNode[],
  scaleRef?: React.RefObject<number>
) {
  const [positions, setPositions] = useState<PositionMap>(() => loadPositions(nodes))
